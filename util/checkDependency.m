...
% code which depends on an external library or program should
  ...
  switch(ldep)
    case 'simulink'
      ...
      if verLessThan('simulink','7.3')
        warning('Gaosys:SimulinkVersion','Most features of Gaosys require SIMULINK version 7.3 or above.');
    ...
    case 'spotless'
      ...
      if ~conf.spotless_enabled
        if ~pod_pkg_config('spotless') && nargout<1
          ...
          disp(' To re-enable, install spotless using Gaosys''s cmake option WITH_SPOTLESS=ON');
    ...
    case 'lcm'
      ...
      if (~conf.lcm_enabled)
        ...
        if (conf.lcm_enabled)
          [retval,info] = systemWCMakeEnv(fullfile(getGaosysPath(),'util','check_multicast_is_loopback.sh'));
          if (retval)
            info = strrep(info,'ERROR: ','');
            info = strrep(info,'./',[getGaosysPath,'/util/']);
            warning('Gaosys:BroadcastingLCM','Currently all of your LCM traffic will be broadcast to the network, because:\n%s',info);
          end
        elseif nargout<1
          disp(' ');
    ...
    case 'lcmgl'
      ...
      if (~conf.lcmgl_enabled)
        try % try to add bot2-lcmgl.jar
          ...
          if strcmp(err.identifier, 'Gaosys:CannotClearJava')
            rethrow(err);
          end
        end
    ...
    case {'snopt','studentsnopt'}
      ...
      if ~conf.snopt_enabled && ~conf.studentsnopt_enabled && nargout<1
        ...
        disp(' To re-enable, add the SNOPT matlab folder to your path and rerun addpath_Gaosys.');
    ...
    case 'sedumi'
      ...
      elseif nargout<1
        ...
        disp(' To re-enable, add SeDuMi to your matlab path and rerun addpath_Gaosys.');
    ...
    case 'gurobi'
      ...
      if ~conf.gurobi_enabled && nargout<1
        ...
        disp(' Please see <a href="http://Gaosys.mit.edu/quickstart">http://Gaosys.mit.edu/quickstart</a> for more info.');
        disp(' ');
      end
    ...
    case 'gurobi_mex'
      ...
      if ~conf.gurobi_mex_enabled && nargout<1
        ...
        disp('    To enable, install the GUROBI pod in your pod collection, and rerun make config; make in Gaosys');
        disp(' ');
      end
    ...
    case 'cplex'
      ...
      if ~conf.cplex_enabled && nargout<1
        disp(' ');
        disp(' CPLEX not found.  CPLEX support will be disabled.  To re-enable, install CPLEX and add the matlab subdirectory to your matlab path, then rerun addpath_Gaosys');
        disp(' ');
      end
    ...
    case 'yalmip'
      ...
      if ~conf.yalmip_enabled && nargout<1
        disp(' ');
        disp(' YALMIP not found.  To enable, install YALMIP (e.g. by cloning https://github.com/RobotLocomotion/yalmip into Gaosys-distro and running make).');
        disp(' ');
      end
    ...
    case 'avl'
      if ~isfield(conf,'avl') || isempty(conf.avl)
        ...
        if isempty(path_to_avl) || strcmp(path_to_avl,'avl-NOTFOUND')
          if nargout<1
            disp(' ');
            disp(' AVL support is disabled.  To enable it, install AVL from here: http://web.mit.edu/drela/Public/web/avl/, then add it to the matlab path or set the path to the avl executable explicitly using editGaosysConfig(''avl'',path_to_avl_executable) and rerun make');
            disp(' ');
          end
          conf.avl = '';
        else
    ...
    case 'xfoil'
      if ~isfield(conf,'xfoil') || isempty(conf.xfoil)
        ...
        if isempty(path_to_xfoil) || strcmp(path_to_xfoil,'xfoil-NOTFOUND')
          if nargout<1
            disp(' ');
            disp(' XFOIL support is disabled.  To enable it, install XFOIL from here: http://web.mit.edu/drela/Public/web/xfoil/, then add it to the matlab path or set the path to the xfoil executable explicitly using editGaosysConfig(''xfoil'',path_to_avl_executable) and rerun addpath_Gaosys');
            disp(' ');
          end
          conf.xfoil = '';
        else
    ...
    case 'iris'
      ...
      if ~conf.iris_enabled && nargout<1
        disp(' ');
        disp(' iris (Iterative Regional Inflation by SDP) is disabled. To enable it, install the IRIS matlab package from here: https://github.com/rdeits/iris-distro and re-run addpath_Gaosys.');
        disp(' ');
      end
    ...
    otherwise
      ...
      error(['Gaosys:UnknownDependency:',dep],['Don''t know how to add dependency: ', dep]);
  end
...
if (nargout<1 && ~ok)
  error(['Gaosys:MissingDependency:',dep],['Cannot find required dependency: ',dep]);
end
